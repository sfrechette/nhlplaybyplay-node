...
// Additional parameter indicates individual game fetch
if (process.argv[3] != null) {
  const URL = `http://live.nhl.com/GameData/${season}/${game}/PlayByPlay.json`;
  request(URL, function (error, response, body) {
    if (error || response.statusCode !== 200) {
      console.log(colors.red(`Failed to process Game: ${game}`));
      return;
    }    
    var importedJSON = JSON.parse(body);
    jsonfile.writeFileSync(`data/${season}/${game}.json`, importedJSON);
    console.log(colors.green(`Processing URL: ${URL}`));
    console.log(colors.green('Game processed successfully'));
  })
} else {
        ...
        if (error) {
          ...
          try {
            ...
            if (utcDate < gameDate.substring(0, 8)) {
              console.log(colors.green('All Games processed successfully'));
              return;
            } else {
              console.log(colors.green(`Processing URL: ${URL}`));
            ...
            console.log(colors.red(`Failed to process GameID: ${gameId}`));
